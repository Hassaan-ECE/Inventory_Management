...
import { InventorySystemPlaceholder } from "@/features/inventory/components/InventorySystemPlaceholder";
...
import {
  getInventorySystem,
  readStoredInventorySystemId,
  writeStoredInventorySystemId,
  type InventorySystemId,
} from "@/features/inventory/lib/inventorySystems";
...
export function InventoryShell() {
  ...
  const statusMessage = !inventoryChrome
    ? `${activeSystem.label} — module not connected yet. Switch systems from the title menu.`
    : isLoading
  ...
  useEffect(() => {
    document.title = `Inventory Management — ${activeSystem.label} v${APP_VERSION}`;
  }, [activeSystem.label]);

  function handleSystemChange(id: InventorySystemId): void {
    setActiveSystemId(id);
    writeStoredInventorySystemId(id);
    setContextMenu(null);
    announceStatus(`Switched to ${getInventorySystem(id).label}.`);
  }
    ...
    <div className="h-screen overflow-hidden bg-background text-foreground">
      <main className="flex h-full min-h-0 flex-col overflow-hidden">
        <InventoryHeader
          ...
          inventoryChrome={inventoryChrome}
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden px-2 py-1.5 sm:px-3">
          {!inventoryChrome ? (
            <InventorySystemPlaceholder system={activeSystem} />
          ) : (
        ...
        <StatusStrip
          counts={inventoryChrome ? counts : undefined}
          message={statusMessage}
          resultsLabel={inventoryChrome ? resultsLabel : activeSystem.label}
        />
      </main>

      {inventoryChrome && contextMenu && contextEntry ? (
        <EntryContextMenu
          canModifyEntries={canModifyEntries}
      ...
      {inventoryChrome && pendingDeleteEntry ? (
