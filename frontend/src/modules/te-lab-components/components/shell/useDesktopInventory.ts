...
import { MOCK_INVENTORY } from "@/modules/te-lab-components/data/mockInventory";
...
export function useDesktopInventory({ announceStatus, active }: UseDesktopInventoryOptions) {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(() => hasDesktopBridge() && active);
  ...
  useEffect(() => {
    ...
    const deactivateSession = (token: string): void => {
      void deactivateInventorySync(TE_LAB_COMPONENTS_MODULE_ID, token).catch(() => undefined);
    };
    ...
    async function activateDesktopInventory(): Promise<void> {
      ...
      let token: string;
      try {
        token = await activateInventorySync(TE_LAB_COMPONENTS_MODULE_ID);
      } catch {
        if (isCurrent()) {
      ...
      unsubscribeSharedChanges = onSharedInventoryChanged?.((payload) => {
        if (payload.systemId === TE_LAB_COMPONENTS_MODULE_ID && isCurrent()) {
          void controller?.requestSync();
        }
      });
