...
| IM-013 | **TE shared-data strategy (long-term):** product default remains `...\Inventory_Management_App\modules\TE_Test_Equipment`. Prefer modularity over making legacy InventoryApps the permanent default. | **Temporary owner validation** may set `INVENTORY_MANAGEMENT_SHARED_ROOT` to `...\InventoryApps\TE_Test_Equipment_Inventory` via `scripts/desktop-legacy-te-share.ps1` so UI can be checked against live data. That override is **not** the product default and must not ship as the team installer default. Before release: copy latest shared data into the product module path and run only Inventory Management as writer. Never dual-write unified + standalone to the same root. Future modules each get `modules\<Name>\`. |
