...
| Multi-inventory backend session map | TE-global session only (design allows future map) |
| Modular monorepo extract (shared platform package layout) | **Not done** (code is still one TE-lineage scaffold) |
...
**Out of A:** full team install, other modules. Next engineering focus after A: **Phase B** (architecture extract) so Lab/ME/Storage ports land cleanly.
...
| C4 | **Session map** | Backend `systemId → session` (from IM-011 future note); hard deactivate per module |
...
**Current default (2026-07-20):** **B1** — monorepo / platform map (IM-012), because A1+A3 are decided for long-term modularity.

| If you care about… | Do next |
|--------------------|---------|
| Long-term structure before Lab/ME ports | **B1** monorepo map (recommended) |
...
**Default recommendation:** **B1** (architecture map) next; optional A2 when convenient; migration import later without dual-writing legacy.
