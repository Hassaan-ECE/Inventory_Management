...
fn scripted_one_machine_smoke_uses_env_shared_root() {
    let Some(smoke_root) = env::var_os("INVENTORY_MANAGEMENT_SYNC_SMOKE_ROOT").map(PathBuf::from)
    else {
        println!(
            "INVENTORY_MANAGEMENT_SYNC_SMOKE_ROOT is not set; skipping script-only smoke scenario."
        );
        return;
    };
