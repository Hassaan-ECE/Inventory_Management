param(
    [string]$SmokeRoot = (Join-Path $env:TEMP "inventory-management-sync-smoke")
)
...
$env:INVENTORY_MANAGEMENT_SYNC_SMOKE_ROOT = $resolvedSmokeRoot
$env:INVENTORY_MANAGEMENT_SHARED_SYNC_ENABLED = "true"

Write-Host "Inventory Management one-machine sync smoke"
...
try {
    ...
    Remove-Item Env:\INVENTORY_MANAGEMENT_SYNC_SMOKE_ROOT -ErrorAction SilentlyContinue
